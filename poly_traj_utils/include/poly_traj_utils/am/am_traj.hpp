...
//            TimeConsuming t__("optimizeCoeffs");
            ...
            if (N == 1) {
                ...
                b.bottomRightCorner<2, 3>()
                        << (-cv00(N - 2) * wayPs[N - 2] - cv01(N - 2) * wayPs[N - 1] - cv02(N - 2) * wayPs[N] -
                            cv12(N - 2) * finVel - cv22(N - 2) * finAcc).transpose(), (-ca00(N - 2) * wayPs[N - 2] -
                                                                                       ca01(N - 2) * wayPs[N - 1] -
                                                                                       ca02(N - 2) * wayPs[N] -
                                                                                       ca12(N - 2) * finVel -
                                                                                       ca22(N - 2) *
                ...
                for (int i = 1; i < N - 2; i++) {
                    ...
                    b.block<2, 3>(i * 2, 0)
                            << (-cv00(i) * wayPs[i] - cv01(i) * wayPs[i + 1] - cv02(i) * wayPs[i + 2]).transpose(), (
                            -ca00(i) * wayPs[i] - ca01(i) * wayPs[i + 1] - ca02(i) * wayPs[i + 2]).transpose();
                }
        ...
        double evaluateObjective(const BoundaryCond &boundCond, double duration) const {
            ...
            coeffsAccObjective(1) = (22.0 * iniAcc * iniVel - 8.0 * finAcc * iniVel + 8.0 * iniAcc * finVel -
                                     22.0 * finAcc * finVel).sum();
            coeffsAccObjective(2) = 6.0 * (32.0 * iniVel.square() + 36.0 * iniVel * finVel + 32.0 * finVel.square() +
                                           5.0 * (iniAcc - finAcc) * (iniPos - finPos)).sum();
            ...
            coeffsJerObjective(1) = 8.0 * (3.0 * iniAcc * iniVel - 2.0 * finAcc * iniVel + 2.0 * iniAcc * finVel -
                                           3.0 * finAcc * finVel).sum();
            coeffsJerObjective(2) = 8.0 * (8.0 * iniVel.square() + 14.0 * iniVel * finVel + 8.0 * finVel.square() +
                                           5.0 * (iniAcc - finAcc) * (iniPos - finPos)).sum();
...
//            TimeConsuming t__("optimizeDurations");
            ...
            for (int i = 0; i < N; i++) {
                ...
                tempJerkTerm = ((-3.0 * accIni + 2.0 * accFin) * velIni +
                                (-2.0 * accIni + 3.0 * accFin) * velFin).sum();
...
//                    }cout<<endl;
                    ...
                    inTol = true;
                    double diffDuration;
                    for (size_t j = 0; j < traj.getPieceNum(); j++) {
                ...
                if (idxPieceStuck != -1) {
                    ...
                    if (idxPieceStuck != traj.getPieceNum() - 1) {
                        for (size_t i = idxPieceStuck + 1; i < traj.getPieceNum(); i++) {
...
//            }cout<<endl;
        ...
        Trajectory genOptimalTrajDTC(Trajectory traj_in) {
            ...
            iniVel = traj_in.getVel(0);
            iniAcc = traj_in.getAcc(0);
            finVel = traj_in.getVel(-1);
            finAcc = traj_in.getAcc(-1);
...
namespace order7 {
    ...
    public:
        ...
        Trajectory genOptimalTrajDTC(Vec3 iniPos, Vec3 iniVel, Vec3 iniAcc, Vec3 iniJerk,
                                     Vec3 finPos, Vec3 finVel, Vec3 finAcc, Vec3 finJerk)  {
            vector<Vec3> way_pts; way_pts.push_back(iniPos);way_pts.push_back(finPos);
        ...
        Trajectory genOptimalTrajDTCFromOrder5(Trajectory traj_in,Vec3 iniVel, Vec3 iniAcc, Vec3 iniJerk,
            ...
            std::vector<Vec3> way_pts = traj_in.getWaypoints();
            way_pts.insert(way_pts.begin(), traj_in.getPos(0));
            way_pts.insert(way_pts.end(), traj_in.getPos(-1));
            /*  Do unconstrained optimization to find the global minimum*/
            std::vector<DynamicMat> coeffMats;
            coeffMats = optimizeCoeffs(way_pts, durations,
                                       iniVel, iniAcc, iniJerk,
